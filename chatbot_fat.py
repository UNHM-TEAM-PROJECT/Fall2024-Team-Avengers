"""
This module implements a Flask-based chatbot application to assist students with 
questions about internships and courses (COMP690 and COMP893). The chatbot uses 
Qdrant for document retrieval and OpenAI's API for generating conversational responses.
...
- PyPDF2, csv for processing and logging
"""
...
app = Flask(__name__)
app.secret_key = "comp690"  # Secret key for session management

@app.route("/")
def hello_world():
    """
    Home route that renders the chatbot user interface.

    Returns:
        str: Rendered HTML template for chatbot UI.
    """
...
def handle_post():
    """
    Handles chatbot interactions via POST and PUT methods.

    POST: Processes user messages, generates responses, and updates session history.
    PUT: Resets the conversation history and session course.

    Returns:
        Response: Chatbot's response to the user or confirmation of reset.
    """
...
def answer_question(messages, chunks):
    """
    Generates an answer to the user's question based on the conversation history 
    ...
    Returns:
        dict: OpenAI response object.
    """
    history = messages.copy()
    loc_chunks = chunks.copy()
    loc_chunks = [mes.payload.values() for x in chunks for mes in x]  # Flatten chunks
...
def main():
    """
    Initializes global variables for the chatbot, including OpenAI and Qdrant clients, 
    embedding model, and chatbot prompt. Starts the Flask application.
    """
    ...
    [Chatbot's guidelines and instructions are specified here]
...
def get_response(session):
    """
    Processes the user's query and generates a response by integrating context 
    retrieval and OpenAI's language model.

    Args:
        session (dict): User's session data containing conversation history and course context.
...
def get_rag(session, question):
    """
    Retrieves relevant context chunks based on the user's query.

    Args:
        session (dict): User's session data.
...
def get_context(messages, question):
    """
    Determines the context of the user's query, categorizing it as COMP690, COMP893, 
    or general, based on the conversation history.
    ...
    Returns:
        str: Identified course context ("COMP690", "COMP893", or "not sure").
    """
...
if __name__ == "__main__":
    """
    Entry point for the chatbot application. Initializes the main logic 
